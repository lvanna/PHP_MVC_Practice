...
<body>
	...
	<div id="add_user_modal" class="modal3">
		  ...
		  <div class="modal-content">
			    ...
			    <form method="POST" action="?action=staff_add" id="staff_add_form">
	      			...
	      			passwd：<input name="passwd" type="text"/><br/> <br/>
	...
	<!-- The 修改Modal -->
	<div id="up_user_modal" class="modal4">
		  <!-- Modal content -->
		  <div class="modal-content">
			    <span class="close4">&times;</span>
			    <form method="POST" action="?action=staff_change" id="staff_change_form">
	...
	<div class="row">
		...
		<div id="divTab" class="col-lg-8">
			...
			<div class="tab" id="TabBig">
				<?php foreach($result as $r) {
			          echo '<button class="tablinks" id="TabBigButton" onclick="openCity(event,\''.$r['did'].'\')">'.$r['dname'].'</button>';
			    }?>
		    </div>
	...
	<?php foreach($result as $r) {
		...
		?>
          ...
          <div id=<?php echo $r['did']?> class="tabcontent tabcontentBig">
				  <br>
				  <?php if($pidresult){?>
				  		<div class="tab">
						...
						<?php foreach($pidresult as $e){?>
							...
							<div id=<?php echo 'tabcontent2'.$e['pid']?> class="tabcontent2">
								<div class="text-right" style="margin-bottom: 10px">
									...
									if($c==1){?>
										<button type="button" id="bt_upd_permission" class="btn btn-success btn-sm" value=<?php echo $r['did'].$e['pid']?> onclick='nowupvalue(this.id,this.value)'>更改<?php echo $e['pname'] ?>權限</button>
									<?php }?>
									<?php if($x==1){?>
										<button type="button" id="bt_add_user" class="btn btn-warning btn-sm" value=<?php echo $r['did'].$e['pid']?> onclick='nowaddvalue(this.id,this.value)'>註冊<?php echo $e['pname'] ?>員工</button>
									<?php }?>
								</div>
					  			 <table class="table">
									    ...
									    <?php if($uid_uname_result){
											foreach($uid_uname_result as $u){?>
												<tr>
													...
													<?php  if($upc==1){?>
 														<td>
															<button class="btn btn-light" id="bt_up_user" value=<?php echo $r['did'].$e['pid']?> onclick='nowupuser(this.id,this.value)'>修改</button>
													    </td>
													<?php }?>
...
<script>
		...
		// Get the modal
		var modal4 = document.getElementById('up_user_modal');
		// Get the button that opens the modal
		var btn4 = document.getElementById("bt_up_user");
		// Get the <span> element that closes the modal
		...
		function nowupuser(clicked_id,clicked_value){
			alert('nowupuser');
			up_user=clicked_value;
			var upuserdid=clicked_value.substr(0, 4);
			alert('upuserdid:'+upuserdid);
			var upuserpid=clicked_value.substr(4, 4);
			alert('upuserpid:'+upuserpid);
